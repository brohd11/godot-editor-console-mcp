# Install the godot-editor-console MCP server into %LOCALAPPDATA%\bin.
#
...
#   $env:VERSION = 'v0.1.0'     pin a release (default: latest)
#
# For flags, use the scriptblock form:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/brohd11/godot-editor-console-mcp/main/install.ps1))) -NoModifyPath
#
# Body below "end config" comes from sh-templates/go/install.template.ps1.
...
function Write-PostInstallNote {
    $installedPath = Join-Path $binDir "$Binary.exe"
    Write-Host @"
...
drop the flag to scope it to just the current project):
  ...
  claude mcp add -s user godot-editor-console -- "$installedPath"
